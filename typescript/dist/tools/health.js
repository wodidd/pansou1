 ...
 * 执行健康检查工具
 */
export async function executeHealthTool(args, httpClient) {
    try {
        // 执行健康检查
 ...
 * 格式化健康检查结果
 */
function formatHealthResult(healthData, serverUrl) {
    ...
    if (healthData.channels && healthData.channels.length > 0) {
        output += `   可用频道:\n`;
        healthData.channels.forEach((channel, index) => {
    ...
    if (healthData.plugins_enabled) {
        ...
        if (healthData.plugins && healthData.plugins.length > 0) {
            ...
            for (let i = 0; i < plugins.length; i += 4) {
                ...
                output += `      ${row.map((plugin, idx) => `${i + idx + 1}. ${plugin}`).join('  ')}\n`;
            }
        }
        else {
            output += `   未发现可用插件\n`;
 ...
 * 格式化错误结果
 */
function formatErrorResult(errorMessage, serverUrl) {
