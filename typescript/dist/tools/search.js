 ...
 * 执行搜索工具
 */
export async function executeSearchTool(args, httpClient) {
    try {
        // 参数验证
 ...
 * 格式化搜索结果
 */
function formatSearchResult(result, keyword, resultType) {
 ...
 * 格式化按网盘类型分组的结果
 */
function formatMergedResults(mergedByType) {
    let output = '';
    const typeNames = {
        'baidu': '百度网盘',
        'aliyun': '阿里云盘',
...
function formatDetailedResults(results) {
    ...
    results.forEach((result, index) => {
        ...
        if (result.links && result.links.length > 0) {
            output += `网盘链接:\n`;
            result.links.forEach((link, linkIndex) => {
