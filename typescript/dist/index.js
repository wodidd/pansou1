...
class PanSouMCPServer {
    constructor() {
        ...
        if (this.config.testDashboard.enabled) {
            this.dashboardServer = new DashboardServer(this.testRunner, this.testStatusStore, this.config.testDashboard);
        }
        this.setupHandlers();
    ...
    setupHandlers() {
        ...
        this.server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
            ...
            try {
                ...
                throw new McpError(ErrorCode.InternalError, `资源读取失败: ${error instanceof Error ? error.message : String(error)}`);
            }
        });
    }
    /**
    ...
    async getPluginsResource() {
        try {
            ...
            throw new McpError(ErrorCode.InternalError, `获取插件信息失败: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
     * 获取频道资源
     */
    async getChannelsResource() {
        try {
            const healthData = await this.httpClient.checkHealth();
            const channels = {
            ...
            throw new McpError(ErrorCode.InternalError, `获取频道信息失败: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
     * 获取网盘类型资源
     */
    async getCloudTypesResource() {
        const cloudTypes = {
            supported: [
                'baidu', // 百度网盘
                'aliyun', // 阿里云盘
