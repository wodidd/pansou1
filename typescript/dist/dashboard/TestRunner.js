...
export class TestRunner extends EventEmitter {
    constructor(config) {
        super();
        this.runningRuns = new Set();
        this.queuedRuns = new Map();
        this.config = config;
    ...
    async runTests(modules) {
        ...
        // Emit queued event
        this.emit('test:queued', { type: 'queued', runId });
        // Save initial status
    ...
    async executeModules(runId, modules) {
        // Update status to running
        this.store.updateStatus(runId, {
            status: 'running',
            startTime: new Date()
        });
        this.emit('test:started', { type: 'started', runId, modules });
        await this.store.saveStatus(runId);
        ...
        for (const module of modules) {
            try {
                this.emit('test:moduleStarted', { type: 'moduleStarted', runId, moduleId: module.id });
                const result = await this.executeModule(runId, module);
                this.store.updateModuleResult(runId, module.id, result);
                if (result.status === 'failed') {
                    hasFailures = true;
                }
                this.emit('test:moduleCompleted', { type: 'moduleCompleted', runId, moduleId: module.id, result });
        ...
        await this.store.saveStatus(runId);
        if (hasFailures) {
            this.emit('test:failed', { type: 'failed', runId, error: overallError || 'Unknown error', status: finalStatus });
        }
        else {
            this.emit('test:completed', { type: 'completed', runId, status: finalStatus });
        }
        return finalStatus;
    }
    async executeModule(runId, module) {
    ...
    async parseTestOutput(module, stdout, stderr, exitCode, duration) {
    ...
    parseGoTestOutput(moduleId, output, duration) {
    ...
    parseJestOutput(moduleId, output, duration) {
        try {
    ...
    parseBasicOutput(moduleId, stdout, stderr, exitCode, duration) {
    ...
    async parseCoverage(runId, modules) {
        for (const module of modules) {
            try {
    ...
    async parseModuleCoverage(module) {
    ...
    parseGoCoverageOutput(moduleId, output) {
        const lines = output.split('\n');
        let totalLines = 0;
        let coveredLines = 0;
        for (const line of lines) {
    ...
    async cancelRun(runId) {
        ...
        if (updatedStatus) {
            this.emit('test:cancelled', { type: 'cancelled', runId, status: updatedStatus });
        }
        await this.store.saveStatus(runId);
        return true;
    }
