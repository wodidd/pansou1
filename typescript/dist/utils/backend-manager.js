...
export class BackendManager {
    constructor(config, httpClient) {
        ...
        this.SHUTDOWN_DELAY = 5000; // 5秒延迟关闭
        this.STARTUP_TIMEOUT = 30000; // 30秒启动超时
        this.HEALTH_CHECK_INTERVAL = 1000; // 1秒健康检查间隔
        this.activityMonitor = null;
        this.config = config;
        this.httpClient = httpClient;
        // 初始化活动监控器
        if (this.config.enableIdleShutdown) {
            this.activityMonitor = new ActivityMonitor(this.config.idleTimeout, this.config.enableIdleShutdown);
            // 设置空闲监控回调
     ...
     * 查找Go可执行文件路径
     */
    async findGoExecutable() {
        // 优先使用配置中的项目根目录
        const configProjectRoot = this.config.projectRootPath;
        const possiblePaths = [];
        // 如果配置了项目根目录，直接在该目录下查找
        if (configProjectRoot) {
            possiblePaths.push(path.join(configProjectRoot, 'pansou.exe'), path.join(configProjectRoot, 'main.exe'));
        }
        else {
            // 仅在没有配置项目根目录时才使用备用路径
            possiblePaths.push(
            // 当前工作目录
            path.join(process.cwd(), 'pansou.exe'), path.join(process.cwd(), 'main.exe'), 
            // 上级目录（如果MCP在子目录中）
            path.join(process.cwd(), '..', 'pansou.exe'), path.join(process.cwd(), '..', 'main.exe'));
        }
 ...
 * 创建后端管理器实例
 */
export function createBackendManager(config, httpClient) {
